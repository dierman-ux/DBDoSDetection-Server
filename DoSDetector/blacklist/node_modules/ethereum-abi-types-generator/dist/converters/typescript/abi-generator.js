...
var AbiGenerator = /** @class */ (function () {
                ...
                case abi_item_type_1.AbiItemType.event:
                    ...
                    if (eventInputs && eventInputs.length > 0) {
                        ...
                        for (var e = 0; e < eventInputs.length; e++) {
                            var eventTsType = helpers_2.default.getSolidityInputTsType(eventInputs[e], this._context.provider, 'EventEmittedResponse');
                            eventTypeProperties += eventInputs[e].name + ": " + eventTsType + ";";
                            if (eventInputs[e].type === abi_properties_1.SolidityType.tuple) {
                                this.buildTupleParametersInterface(
                                // remove due to prefix of components
                                eventTsType.replace('EventEmittedResponse', ''), eventInputs[e], 'EventEmittedResponse');
                            }
                        }
        ...
        if (abiItem.inputs) {
            for (var i = 0; i < abiItem.inputs.length; i++) {
                ...
                if (abiItem.inputs[i].type.includes(abi_properties_1.SolidityType.tuple)) {
                    input += inputName + ": " + this.buildTupleParametersInterface(inputName, abiItem.inputs[i]);
                }
                else {
     ...
     */
    AbiGenerator.prototype.buildTupleParametersInterface = function (name, abiInput, prefix) {
        if (prefix === void 0) { prefix = 'Request'; }
        var interfaceName = "" + helpers_1.default.capitalize(name) + prefix;
        var properties = '';
        for (var i = 0; i < abiInput.components.length; i++) {
            ...
            if (abiInput.components[i].components) {
                var deepInterfaceName = helpers_2.default.buildInterfaceName(abiInput.components[i], prefix);
                var deepProperties = '';
