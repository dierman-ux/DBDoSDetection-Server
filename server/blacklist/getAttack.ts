...
async function getAttack(n: number = 0) {
  // inicializa cliente Thor
  ...
  const attacks = await contract.read.getAttack(n);

  const count = attacks[0].toString();
  console.log(`IP Associated to Attack ${n}: ${count}`);
}
