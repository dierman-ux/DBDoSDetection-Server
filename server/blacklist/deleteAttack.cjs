...
function deleteAttack(index) {
    const deleteClause = contract.clause.deleteAttack(index);
    ...
    return __awaiter(this, void 0, void 0, function () {
        var gasResult, txBody, signer, rawSignedTransaction, signedTransaction, sendTransactionResult;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    return [4 /*yield*/, thorSoloClient.transactions.estimateGas(transaction.clauses, transaction.simulateTransactionOptions.caller)];
                case 1:
                    gasResult = _a.sent();
                ...
                case 3:
                    ...
                    return [4 /*yield*/, signer.signTransaction(
                        sdk_network_2.signerUtils.transactionBodyToTransactionRequestInput(txBody, senderAccount.address)
                    )];
                case 4:
                    console.log("Signing transaction...");
                    rawSignedTransaction = _a.sent();

                    signedTransaction = sdk_core_1.Transaction.decode(
                        sdk_core_1.HexUInt.of(rawSignedTransaction.slice(2)).bytes,
                        true
                    );
...
module.exports = { deleteAttack };

// Si se ejecuta directamente desde línea de comandos
if (require.main === module) {
    const [, , indexArg] = process.argv;

    if (!indexArg || isNaN(indexArg)) {
        console.error("Uso: node deleteAttack.js <índice>");
        process.exit(1);
    }

    deleteAttack(Number(indexArg))
        .then(() => console.log("Ataque eliminado correctamente"))
        .catch((err) => console.error("Error al eliminar el ataque:", err));
}
